# v17 to v17-b Testnet Upgrade Guide
...
Create the v17b folder, make the build, and copy the daemon over to that folder

```{.sh}
mkdir -p ~/.osmosisd/cosmovisor/upgrades/v17b/bin
cd $HOME/osmosis
git pull
git checkout v17b-testnet
make build
cp build/osmosisd ~/.osmosisd/cosmovisor/upgrades/v17b/bin
```
...
1. Wait for Osmosis to reach the upgrade height (2217000)
