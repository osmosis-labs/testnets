# v30 to v31 Testnet Upgrade Guide

|                 |                                                            |
|-----------------|------------------------------------------------------------|
| Chain-id        | `osmo-test-5`                                              |
| Upgrade Version | `v31.0.0-rc0`                                              |
| Upgrade Height  | 38420500                                                   |
| Countdown       | <https://www.mintscan.io/osmosis-testnet/block/38420500>   |
...
Copy the current v31 osmosisd binary into the
cosmovisor/genesis folder and v30 folder.

```{.sh}
cp $GOPATH/bin/osmosisd ~/.osmosisd/cosmovisor/genesis/bin
mkdir -p ~/.osmosisd/cosmovisor/upgrades/v30/bin
cp $GOPATH/bin/osmosisd ~/.osmosisd/cosmovisor/upgrades/v30/bin
```

Cosmovisor is now ready to be set up for v30.
...
Create the v31 folder, make the build, and copy the daemon over to that folder

```{.sh}
mkdir -p ~/.osmosisd/cosmovisor/upgrades/v31/bin
cd $HOME/osmosis
git pull
git checkout v31.0.0-rc0
make build
cp build/osmosisd ~/.osmosisd/cosmovisor/upgrades/v31/bin
```

Now, at the upgrade height, Cosmovisor will upgrade to the v31 binary

## Manual Option

1. Wait for Osmosis to reach the upgrade height (38420500)
